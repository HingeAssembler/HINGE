...
for (int i = 0; i < n_read; i ++) {
    ...
    la.profileCoverage(idx3[i],coverage,40,0);
    cov << "read " << i <<" ";
...
/** look for missing edges **/
    ...
    for (int i = 0; i < edgelist.size(); i++){
	    ...
	    la.profileCoverage(idx4[n2.id],coverage,40,0);
	    ...
	    la.profileCoverage(idx4[n1.id],coverage1,40,0); // profile coverage and look for unusual pattern
		for (int j = 10; j < 20; j++) {
